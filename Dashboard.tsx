...
export function Dashboard() {
  ...
  const takeAction = (id: string) => navigate(`/dashboard/revenue-recovery?opportunity=${encodeURIComponent(id)}`)
          ...
          <div>
            {queue.map((opp, index) => (
              <div key={opp.id} className={`grid grid-cols-[minmax(125px,180px)_minmax(0,1fr)_auto_auto_auto] gap-3 items-center px-6 py-4 ${index !== queue.length - 1 ? 'border-b border-slate-800' : ''}`}>
                ...
                <button onClick={() => takeAction(opp.id)} className="rounded-lg bg-brand-500 hover:bg-brand-400 text-slate-950 font-semibold text-xs px-4 py-2.5 whitespace-nowrap transition-colors">Take action</button>
              </div>
            ))}
