...
export function DashboardLayout({ children, currentPath }: { children: ReactNode; currentPath: string }) {
  ...
  return <div className="min-h-screen bg-slate-50 flex">
    ...
    <div className="flex-1 lg:ml-64 flex flex-col min-w-0">
      <header className="lg:hidden bg-slate-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-20"><button onClick={() => setMobileOpen(true)} className="p-1.5 rounded-lg hover:bg-slate-800"><Menu className="w-6 h-6" /></button><div className="flex items-center gap-2"><Zap className="w-5 h-5 text-brand-400" /><span className="font-bold text-sm tracking-tight">Loose Ends Co.</span></div><span className="badge bg-brand-500 text-white text-[10px]">CRM</span></header>
      <div className="hidden lg:flex h-16 items-center justify-between border-b border-slate-200 bg-white px-8 sticky top-0 z-20"><div className="flex items-center gap-2 text-sm text-slate-500"><span>Workspace</span><ChevronRight className="w-4 h-4" /><span className="font-semibold text-slate-900">{navItems.find(item => item.path === currentPath || (item.path !== '/dashboard' && currentPath.startsWith(item.path)))?.label || 'Dashboard'}</span></div><div className="flex items-center gap-3"><div className="hidden xl:flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500 focus-within:bg-white focus-within:border-brand-300"><Search className="w-3.5 h-3.5" /><input ref={searchRef} aria-label="Search CRM" onKeyDown={e => { if (e.key === 'Enter') navigate('/dashboard/crm') }} className="bg-transparent outline-none w-32 placeholder:text-slate-400" placeholder="Search CRM" /><kbd className="rounded border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[10px]">/</kbd></div><span className="badge bg-brand-100 text-brand-700">CRM</span></div></div>
