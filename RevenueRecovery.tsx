  ...
  return (
    <div className="space-y-6 animate-fadeIn">
          ...
          {PIPELINE_STAGES.map((stage, i) => (
            <div key={stage} className="flex items-center flex-shrink-0">
              <div className="text-center">
                <div className={`px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap ${pipelineValues[stage] > 0 ? 'bg-brand-100 text-brand-700' : 'bg-slate-100 text-slate-400'}`}>
                  {stage}
                </div>
      ...
      <div className="card p-4">
        ...
        <div className="flex flex-wrap gap-2">
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="input w-auto">
            {OPPORTUNITY_TYPES.map(t => <option key={t} value={t}>{t === 'All' ? 'All Types' : t}</option>)}
          </select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map(opp => (
          <div key={opp.id} className="card p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-bold text-slate-900">{opp.customerName}</h3>
                  <OpportunityTypeBadge type={opp.type} />
                </div>
                <p className="text-xs text-slate-500 mt-1">
                  Identified {formatDate(opp.dateIdentified)}
                  {opp.lastContact && ` · Last contact ${timeAgo(opp.lastContact)}`}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                {opp.status === 'Collected' && opp.collectedAmount ? (
                  <>
                    <p className="text-lg font-bold text-success-600">{formatCurrency(opp.collectedAmount)}</p>
                    <p className="text-[10px] text-slate-400 line-through">{formatCurrency(opp.potentialValue)} est.</p>
                  </>
                ) : (
                  <p className="text-lg font-bold text-brand-600">{formatCurrency(opp.potentialValue)}</p>
                )}
            ...
            <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
              <button
                onClick={() => updateOpportunity(opp.id, { status: 'Contacted', lastContact: new Date().toISOString() })}
                className="btn-ghost text-xs px-2.5 py-1.5"
                disabled={opp.status !== 'Potential'}
              >
                <Check className="w-3.5 h-3.5" /> Contact
              </button>
              <button
                onClick={() => updateOpportunity(opp.id, { status: 'Scheduled' })}
                className="btn-ghost text-xs px-2.5 py-1.5"
                disabled={opp.status === 'Collected' || opp.status === 'Completed'}
              >
                <CalendarClock className="w-3.5 h-3.5" /> Schedule
              </button>
              {opp.status !== 'Collected' && (
                <button onClick={() => advanceStage(opp)} className="btn-secondary text-xs px-2.5 py-1.5">
                  <ChevronRight className="w-3.5 h-3.5" />
                  {opp.status === 'Completed' ? 'Mark Collected' : 'Advance'}
                </button>
              )}
              <button onClick={() => snooze(opp)} className="btn-ghost text-xs px-2.5 py-1.5 ml-auto">
        ...
        {filtered.length === 0 && (
          <div className="col-span-full card p-8 text-center text-slate-500">No opportunities match the current filters.</div>
        )}
      </div>
      ...
      {collectModal && (
        <div
          ...
          onClick={() => { setCollectModal(null); setCollectAmount('') }}
        >
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-900">Confirm Collected Revenue</h3>
              <button onClick={() => { setCollectModal(null); setCollectAmount('') }} className="p-1 rounded-lg hover:bg-slate-100">
                <X className="w-5 h-5 text-slate-400" />
              </button>
            </div>
            <p className="text-sm text-slate-600 mb-1">
              <span className="font-semibold">{collectModal.customerName}</span> — {collectModal.type}
            </p>
            <p className="text-xs text-slate-500 mb-4">Estimated potential: {formatCurrency(collectModal.potentialValue)}</p>
            <div className="mb-4">
              <label className="label">Confirmed Revenue Amount</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0"
                  autoFocus
                  onKeyDown={e => e.key === 'Enter' && confirmCollect()}
                />
              </div>
              <p className="text-xs text-slate-500 mt-1.5">
                Enter the actual amount collected from the customer. Only this confirmed amount counts toward Recovered Revenue.
              </p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => { setCollectModal(null); setCollectAmount('') }} className="btn-secondary flex-1">Cancel</button>
              <button
