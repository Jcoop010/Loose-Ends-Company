...
(function(){
  ...
  var db=load();
  function toast(m){var el=document.getElementById('toast');if(!el)return;el.textContent=m;el.className='toast show';setTimeout(function(){el.className='toast';},2000);}
  function mark(view){
    document.querySelectorAll('.nav-item').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-view')===view);});
  }
  function go(view){
    mark(view);
    if(view==='sales'){renderSales();return;}
    if(view==='calendar'){renderCal();return;}
    if(window.LE && LE.__salesOrigSetView){LE.__salesOrigSetView(view);return;}
  ...
  function renderSales(){
    ...
    var html='<div class="page">'+head('Sales Orders','Quote, schedule, invoice, and collect work from one pipeline.','SALES')+
      '<div class="grid4">'+kpi('Open orders',String(open.length),'Not paid or cancelled')+kpi('Pipeline',money(pipe),'Open order value')+kpi('Paid',String(db.orders.filter(function(o){return o.pay==='Paid';}).length),'Collected')+kpi('Events',String(db.events.length),'On the calendar')+'</div>'+
      '<div class="toolbar" style="display:flex;gap:8px;flex-wrap:wrap;margin:14px 0"><input id="soSearch" class="input" placeholder="Search orders..." style="flex:1"><select id="soFilter" class="input">'+['All'].concat(STATUSES).map(function(s){return '<option>'+s+'</option>';}).join('')+'</select><button class="primary" id="soNew" type="button">+ New order</button></div>'+
    ...
    var neu=qs('#soNew');if(neu)neu.onclick=function(){editOrder(null);};
    function run(){
      var q=((qs('#soSearch')||{}).value||'').toLowerCase(),f=(qs('#soFilter')||{}).value||'All';
    ...
    if(qs('#soSearch'))qs('#soSearch').oninput=run;
    if(qs('#soFilter'))qs('#soFilter').onchange=run;
    document.querySelectorAll('.le-card').forEach(function(c){c.onclick=function(){editOrder(c.getAttribute('data-id'));};});
  }
  function card(o){
    return '<button type="button" class="le-card" data-id="'+o.id+'" data-status="'+esc(o.status)+'" data-text="'+esc((o.number+' '+o.customer+' '+o.title+' '+o.status).toLowerCase())+'"><small>'+esc(o.number)+'</small><strong>'+esc(o.customer)+'</strong><span>'+esc(o.title)+'</span><b>'+money(total(o))+(o.due?' · '+o.due:'')+'</b></button>';
  }
  function editOrder(id){
    ...
    var items=o&&o.items&&o.items.length?o.items.map(function(x){return {name:x.name,qty:x.qty,price:x.price};}):[{name:'',qty:1,price:0}];
    wrap.innerHTML='<div class="sales-modal" style="max-width:680px"><div class="sales-modal-head"><div><div class="eyebrow">SALES ORDER</div><h2>'+(o?esc(o.number):'New order')+'</h2></div><button type="button" class="sales-close">×</button></div>'+
      ...
      '<div id="soLines"></div><button type="button" class="action secondary" id="soAddLine">Add line</button>'+
      '<div class="audit-foot"><strong id="soTot"></strong>'+(o?'<button type="button" class="action secondary" id="soDel">Delete</button><button type="button" class="action secondary" id="soCal">Add to calendar</button>':'')+'<button type="button" class="primary" id="soSave">Save order</button></div></div>';
    document.body.appendChild(wrap);
    function paint(){
      qs('#soLines').innerHTML=items.map(function(it,i){return '<div class="le-line"><input data-k="name" data-i="'+i+'" placeholder="Item" value="'+esc(it.name)+'"><input data-k="qty" data-i="'+i+'" type="number" value="'+it.qty+'"><input data-k="price" data-i="'+i+'" type="number" value="'+it.price+'"><button type="button" data-del="'+i+'">×</button></div>';}).join('');
      qs('#soTot').textContent='Total '+money(items.reduce(function(a,i){return a+(Number(i.qty)||0)*(Number(i.price)||0);},0)*1.07);
    ...
    qs('#soSave').onclick=function(){
      var rec={id:o?o.id:uid(),number:o?o.number:'SO-'+(1000+db.orders.length+1),customer:qs('#soCust').value||'Customer',title:qs('#soTitle').value||'Untitled',status:qs('#soStatus').value,pay:qs('#soPay').value,due:qs('#soDue').value,when:qs('#soWhen').value,tax:0.07,items:items.filter(function(i){return i.name;})};
      if(o)db.orders=db.orders.map(function(x){return x.id===o.id?rec:x;});else db.orders.unshift(rec);
      persist(db);wrap.remove();toast('Order saved');renderSales();
    };
    if(qs('#soDel'))qs('#soDel').onclick=function(){db.orders=db.orders.filter(function(x){return x.id!==o.id;});persist(db);wrap.remove();toast('Order deleted');renderSales();};
    if(qs('#soCal'))qs('#soCal').onclick=function(){
      ...
      persist(db);toast('Added to calendar');
    };
  }
  function renderCal(){
    ...
    var html='<div class="page">'+head('Calendar','Appointments, sales-order dates, calls, and follow-ups.','SCHEDULE')+
      '<div style="display:flex;gap:8px;align-items:center;margin-bottom:14px"><button type="button" class="action secondary" id="calPrev">‹</button><strong>'+first.toLocaleString('en-US',{month:'long',year:'numeric'})+'</strong><button type="button" class="action secondary" id="calNext">›</button><button type="button" class="primary" id="calNew" style="margin-left:auto">+ New event</button></div>'+
      '<div class="two-col"><div class="card" style="padding:10px"><div class="le-week">'+['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(function(d){return '<div>'+d+'</div>';}).join('')+'</div><div class="le-grid">'+days.map(function(d){
        ...
        return '<button type="button" class="le-day'+(k===selKey?' on':'')+(inM?'':' dim')+'" data-day="'+k+'"><i>'+d.getDate()+'</i>'+evs.slice(0,3).map(function(e){return '<em>'+esc(e.title)+'</em>';}).join('')+'</button>';
      }).join('')+'</div></div><div class="card section-card"><div class="section-title">'+selected.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'})+'</div><div class="section-sub">'+list.length+' scheduled</div>'+list.map(function(e){return '<div class="activity-row"><div class="act-ico">▦</div><div><strong>'+esc(e.title)+'</strong><small>'+esc(e.type)+(e.who?' · '+esc(e.who):'')+'</small></div>'+(String(e.id).indexOf('so-')===0?'':'<button type="button" class="link-btn" data-kill="'+e.id+'">Remove</button>')+'</div>';}).join('')+(list.length?'':'<p class="muted">Nothing scheduled.</p>')+'</div></div></div>';
    var el=document.getElementById('content');if(!el)return;el.innerHTML=html;
    ...
    document.querySelectorAll('[data-kill]').forEach(function(b){b.onclick=function(){db.events=db.events.filter(function(e){return e.id!==b.getAttribute('data-kill');});persist(db);renderCal();};});
  }
  function editEvent(day){
    var wrap=document.createElement('div');wrap.className='sales-modal-wrap';
    wrap.innerHTML='<div class="sales-modal"><div class="sales-modal-head"><h2>New event</h2><button type="button" class="sales-close">×</button></div><div class="audit-grid"><div class="audit-field"><label>Title</label><input id="evTitle"></div><div class="audit-field"><label>Type</label><select id="evType">'+TYPES.map(function(t){return '<option>'+t+'</option>';}).join('')+'</select></div><div class="audit-field"><label>Customer</label><input id="evWho"></div><div class="audit-field"><label>Start</label><input id="evStart" type="datetime-local" value="'+day+'T09:00"></div></div><div class="audit-foot"><button type="button" class="primary" id="evSave">Save event</button></div></div>';
    document.body.appendChild(wrap);
    qs('.sales-close',wrap).onclick=function(){wrap.remove();};
    qs('#evSave').onclick=function(){
      ...
      persist(db);wrap.remove();toast('Event saved');renderCal();
    };
  }
  function onNavClick(e){
    var n=e.target&&e.target.closest?e.target.closest('[data-view]'):null;
    if(!n)return;
    ...
    if(view==='customers'||view==='overview'||view==='loose'||view==='recovery'||view==='intelligence'||view==='settings'){
      mark(view);
      if(window.LE&&LE.__salesOrigSetView){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();LE.__salesOrigSetView(view);}
    }
  }
  function hook(){
    if(window.LE&&LE.setView&&!LE.__salesHooked){
      LE.__salesOrigSetView=LE.setView.bind(LE);
      LE.setView=function(v){if(v==='sales'||v==='calendar'){go(v);return;}return LE.__salesOrigSetView(v);};
      LE.__salesHooked=true;
      window.setView=LE.setView;
    }
    if(!window.__leNavCapture){
      window.__leNavCapture=true;
